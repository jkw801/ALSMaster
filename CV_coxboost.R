...
movement_merge3 <- merge(movement_survival_sub,allpatientsfeature_imputed_nonscaled,all.x=TRUE)
movement_final_sub<- subset(movement_merge3, select=-SubjectID)
swallowing_merge3 <- merge(swallowing_survival_sub,allpatientsfeature_imputed_nonscaled,all.x=TRUE)
swallowing_final_sub<- subset(swallowing_merge3, select=-SubjectID)
communicating_merge3 <- merge(communicating_survival_sub,allpatientsfeature_imputed_nonscaled,all.x=TRUE)
communicating_final_sub<- subset(communicating_merge3, select=-SubjectID)
breathing_merge3 <- merge(breathing_survival_sub,allpatientsfeature_imputed_nonscaled,all.x=TRUE)
...
for (i in 1:5){
  ...
  print(i)
  model=CoxBoost(training$movement_whenevent,training$movement_isevent,model.matrix(~.-1,training[,-c(1,2)]))
  v1[i]=1-survConcordance(Surv(movement_whenevent,movement_isevent)~predict(model,model.matrix(~.-1,test[,-c(1,2)]),times=365,type="risk"),test)$concordance
}
...
for (i in 1:5){
  ...
  print(i)
  model=CoxBoost(training$swallowing_whenevent,training$swallowing_isevent,model.matrix(~.-1,training[,-c(1,2)]))
  v2[i]=1-survConcordance(Surv(swallowing_whenevent,swallowing_isevent)~predict(model,model.matrix(~.-1,test[,-c(1,2)]),times=365,type="risk"),test)$concordance
...
for (i in 1:5){
  ...
  print(i)
  model=CoxBoost(training$communicating_whenevent,training$communicating_isevent,model.matrix(~.-1,training[,-c(1,2)]))
  v3[i]=1-survConcordance(Surv(communicating_whenevent,communicating_isevent)~predict(model,model.matrix(~.-1,test[,-c(1,2)]),times=365,type="risk"),test)$concordance
...
for (i in 1:5){
  ...
  print(i)
  model=CoxBoost(training$breathing_whenevent,training$breathing_isevent,model.matrix(~.-1,training[,-c(1,2)]))
  v4[i]=1-survConcordance(Surv(breathing_whenevent,breathing_isevent)~predict(model,model.matrix(~.-1,test[,-c(1,2)]),times=365,type="risk"),test)$concordance
...
for (i in 1:5){
  ...
  print(i)
  model=CoxBoost(training$when,training$is,model.matrix(~.-1,training[,-c(1,2)]))
  v5[i]=1-survConcordance(Surv(when,is)~predict(model,model.matrix(~.-1,test[,-c(1,2)]),times=365,type="risk"),test)$concordance
